...
func (driver *Driver) NodeStageVolume(ctx context.Context, req *csi.NodeStageVolumeRequest) (*csi.NodeStageVolumeResponse, error) {
	...
	client := ExtractClientType(volContext, secrets, IRODSFuseType)

	switch client {
	case IRODSFuseType:
		klog.V(5).Infof("NodeStageVolume: mounting %s", string(client))
		if err := driver.mountIRODSFuse(volContext, secrets, mountOptions, targetPath); err != nil {
	...
	default:
		return nil, status.Errorf(codes.Internal, "unknown driver type - %v", string(client))
	}
...
func (driver *Driver) mountIRODSFuse(volContext map[string]string, volSecrets map[string]string, mntOptions []string, targetPath string) error {
	irodsConn, err := ExtractIRODSFuseConnection(volContext, volSecrets)
	if err != nil {
		return err
	}
	...
	fsType := "irodsfs"
	urlSource, err := url.Parse(irodsConn.URL)
	if err != nil {
		return err
	}

	urlSource.User = url.User(irodsConn.User)

	source := urlSource.String()
...
func (driver *Driver) mountWebdav(volContext map[string]string, volSecrets map[string]string, mntOptions []string, targetPath string) error {
	irodsConn, err := ExtractWebDAVConnection(volContext, volSecrets)
	if err != nil {
		return err
	}
